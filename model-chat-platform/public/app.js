...
function save() { localStorage.setItem("modelHubConfig", JSON.stringify({ baseUrl: $("baseUrl").value, apiKey: $("apiKey").value, model: selectedModel(), mode: selectedMode(), temperature: $("temperature").value, maxTokens: $("maxTokens").value, outputTokenDefaultVersion: 2 })); }
...
try { const config = JSON.parse(localStorage.getItem("modelHubConfig") || "{}"); if (!config.outputTokenDefaultVersion && Number(config.maxTokens) === 2048) config.maxTokens = 8192; for (const key of ["baseUrl", "apiKey", "model", "mode", "temperature", "maxTokens"]) if (config[key] != null && $(key)) $(key).value = config[key]; } catch {}
updateModeDisplay();
