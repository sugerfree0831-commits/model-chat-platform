...
async function handler(req, res) {
  ...
  const functionRoute = url.pathname === "/.netlify/functions/api";
  if (req.method === "GET" && (url.pathname === "/api/models" || (functionRoute && url.searchParams.get("action") === "models"))) return json(res, 200, { models: JSON.parse(process.env.TEAM_MODELS || "[{\"value\":\"gpt-6-astra\",\"label\":\"6 Astra\"},{\"value\":\"gpt-5.6-sol\",\"label\":\"5.6 Sol\"},{\"value\":\"gpt-5.6-terra\",\"label\":\"5.6 Terra\"},{\"value\":\"gpt-5.6-luna\",\"label\":\"5.6 Luna\"}]") });
  if (req.method === "POST" && (url.pathname === "/api/chat" || functionRoute)) {
    try {
      ...
      const response = await upstream(/\/chat\/completions$/.test(base) ? base : `${base}/chat/completions`, { model, messages: requestMessages, temperature: Number.isFinite(+data.temperature) ? +data.temperature : 0.7, max_tokens: outputTokenLimit(data.maxTokens), stream: true }, key);
      ...
      if ((response.headers.get("content-type") || "").includes("text/event-stream") && response.body) { res.writeHead(200, { "Content-Type": "text/event-stream; charset=utf-8", "Cache-Control": "no-cache, no-transform", Connection: "keep-alive", "X-Accel-Buffering": "no", "X-Model": model }); const keepAlive = setInterval(() => res.write(": keep-alive\n\n"), 4000); try { for await (const chunk of response.body) res.write(chunk); } finally { clearInterval(keepAlive); } return res.end(); }
      const raw = await response.text(); if (/^\s*<(?:!doctype|html)/i.test(raw)) return json(res, 502, { error: "上游返回了 HTML 错误页", errorType: "HTML_ERROR_PAGE" }); const payload = JSON.parse(raw); return json(res, 200, { content: payload?.choices?.[0]?.message?.content || payload?.output_text || "", model: payload?.model || model, finishReason: payload?.choices?.[0]?.finish_reason || payload?.stop_reason || null, usage: payload?.usage || null }, { "X-Model": payload?.model || model });
